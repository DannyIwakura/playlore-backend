...
- `jwt.secret` NO tiene fallback: `JwtUtils` lanza `IllegalArgumentException` al arrancar si está en blanco (fail-fast).
...
- PROHIBIDOS `alert()`/`confirm()` nativos en el frontend; usar `ModalAviso.vue`/`ModalConfirmacion.vue`.
- Ver `AGENTS.md` del frontend en `E:\playlore-frontend\playlore-frontend\`.
